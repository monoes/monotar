...
Agents do not share a live mind or converge on a state. What actually exists
is **persistent storage that any agent can write to and later read back**,
and your job is to curate it well.

- **No timers.** You run when invoked and stop when you return — not a daemon.
- **No cognitive-load monitoring.** Nothing measures another agent's capacity,
  so tasks cannot be "redistributed based on load".
- **No quorum recovery.** Conflict resolution here is you reading two claims
  and deciding which the evidence supports.
