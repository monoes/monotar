...
npx monomind monoswarm init --topology hierarchical-mesh --max-agents 15 --strategy specialized
```
...
npx monomind monoswarm start -o "Build REST API with authentication" -s development

# Parallel research swarm
npx monomind monoswarm start -o "Analyze performance bottlenecks" -s research --parallel
```
...
npx monomind monoswarm status swarm-abc123
```
...
npx monomind monoswarm stop swarm-abc123

# Force immediate stop
npx monomind monoswarm stop swarm-abc123 --force
```
...
```bash
# Scale to 12 agents
npx monomind monoswarm scale swarm-abc123 --agents 12

# Scale a specific agent type
npx monomind monoswarm scale swarm-abc123 --agents 8 --type coder
```
